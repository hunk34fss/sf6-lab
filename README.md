...
1. アプリの「対戦入力」タブを開く
2. フォーム下部の「OCR取込」ボタンを押してダイアログを開く
3. ランクマッチ結果画面のスクリーンショットを用意する
4. 「画像ファイルを選択」または「クリップボードから貼り付け」（Ctrl+V 也可）
5. 解析結果を確認し、「フォームに反映」（ダイアログは自動で閉じます）
6. 必要なら手動修正して「登録」

「閉じる」ボタンでもダイアログを閉じられます。ホットキーによるSF6ウィンドウ自動キャプチャは未実装（次フェーズ予定）です。
